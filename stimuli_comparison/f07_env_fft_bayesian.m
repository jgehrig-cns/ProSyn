function f07_env_fft_bayesian(paths,ps)
input_root_path = paths.envelopes;
output_root_path = paths.env_freq_bayesian;

%% loading envelop data ...
load(fullfile(input_root_path,'info4bayesian.mat'));

%% locating sig frex ...
p_sig_idx = [find(p_info<=ps.sig);norm_frex(p_info<=ps.sig)]';
fig_idx = 1;
for i=1:size(p_sig_idx,1)
    tmp_freq_idx = p_sig_idx(i,1);
    tmp_freq = p_sig_idx(i,2);
    tmp_data = squeeze(data4statistics(tmp_freq_idx,:,:));
    for j=1:3 %
        %% . cannot be valid str ... 
        if tmp_freq ~= round(tmp_freq)
            tmp_freq = strrep(num2str(tmp_freq),'.','p');
        else
            tmp_freq = num2str(tmp_freq);
        end
        ...
        switch j
            case 1
                %% IT vs TN
                tmp_inference_data = tmp_data(:,1:2); % check ps.conds ...
                summary = bayesian_two_samples (tmp_inference_data(:,1)',tmp_inference_data(:,2)',['IT vs TN: ',num2str(tmp_freq),' Hz']);
                print(gcf,fullfile(output_root_path,['Fig',sprintf('%02d',fig_idx),'_',tmp_freq,'_hz_IT_vs_TN']),'-dtiff','-r600');
                close all;
            case 2
                %% IT vs TT
                tmp_inference_data = tmp_data(:,[1,3]); % check ps.conds ...
                summary = bayesian_two_samples (tmp_inference_data(:,1)',tmp_inference_data(:,2)',['IT vs TT: ',num2str(tmp_freq),' Hz']);
                print(gcf,fullfile(output_root_path,['Fig',sprintf('%02d',fig_idx),'_',tmp_freq,'_hz_IT_vs_TT']),'-dtiff','-r600');
                close all;
            case 3
                %% TN vs TT
                tmp_inference_data = tmp_data(:,2:3); % check ps.conds ...
                summary = bayesian_two_samples (tmp_inference_data(:,1)',tmp_inference_data(:,2)',['TN vs TT: ',num2str(tmp_freq),' Hz']);
                print(gcf,fullfile(output_root_path,['Fig',sprintf('%02d',fig_idx),'_',tmp_freq,'_hz_TN_vs_TT']),'-dtiff','-r600');
                close all;
        end
        fig_idx = fig_idx + 1;       
    end
...
%% inference on sig bins ...

function [summary] = bayesian_two_samples (y1,y2,title)

% function [summary,h_summary] = bayesian_two_samples (y1,y2,title)
...
%% Examine the chains
% mbe_diagMCMC(mcmcChain); % chain plot ...
...
% h_summary = mbe_2gr_plots(data,mcmcChain,'plotPairs',0);

mu1 = mcmcChain.mu1;
mu2 = mcmcChain.mu2;
mbe_plotPost(mu1-mu2,'xlab',strrep(title(1:8),'vs','-'),'ylab','','PlotTitle',title,'CompVal',0,'showMode',1);
